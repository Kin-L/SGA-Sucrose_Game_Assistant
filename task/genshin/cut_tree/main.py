...
class CutTree(Mondstadt, LiYue, Inazuma, Sumeru, Fontaine):
    def genshin_cut_tree(self):
        ...
        click(1053, 48)
        wait(800)
        (x, y), val = find_pic(r"resource\genshin\picture\lit_tools\boon_elder_tree.png",
                               (110, 112, 1273, 805))
        if val < 0.75:
            self.indicate("没有找到道具：王树瑞佑")
            self.indicate("采集木材结束")
            return 0
        else:
            click(x, y)
            wait(800)
            if find_pic(r"resource\genshin\picture\lit_tools\unload.png",
